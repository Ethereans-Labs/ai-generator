...
function setBalance(stateManager, address, subject, balance, provider, tokenOnly) {
    ...
    if (address !== VOID_ETHEREUM_ADDRESS) {
        var pr = stateManager.putContractStorage({ buf: Buffer.from(address.substring(2), 'hex') }, getSlot(subject), balanceBuffer);
...
async function getBalance(stateManager, address, subject) {
    ...
    if (address === VOID_ETHEREUM_ADDRESS) {
        ...
        balance = await stateManager.getContractStorage({ buf: Buffer.from(address.substring(2), 'hex') }, getSlot(subject));
    }
...
async function setL1BlockNumber(stateManager, l2BlockNumber, provider) {
    ...
    var l1BlockNumber = await stateManager.getContractStorage(iOVM_L1BlockNumber_address, l2BlockNumberSlot);
    ...
    await stateManager.putContractStorage(iOVM_L1BlockNumber_address, l2BlockNumberSlot, web3Utils.toBN(l1BlockNumber).toBuffer());
